...
LVGL 软件渲染
  ...
  -> GC9A01 2A/2B/2C 命令
  -> TFT_writePixels()
  -> SPI1 阻塞发送 RGB565 高字节、低字节
  -> lv_display_flush_ready()
```

当前没有 DMA，flush 返回前数据已经发送完成，因此可以立即通知 LVGL 刷新结束。
...
- UI 任务栈：1024 个 `StackType_t`，RV32 下约 4 KB，来自 FreeRTOS 堆。
- LVGL 绘图缓冲：4800 字节静态数组。
- 最近构建：`text=221128`、`data=336`、`bss=36716`。

内部 Flash 空间已经较紧张。新增资源时优先使用字体子集；大字体和图片后续应存放到 W25Q128，并通过 LVGL 文件系统读取。
